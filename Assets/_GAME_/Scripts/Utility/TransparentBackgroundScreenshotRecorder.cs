...
void SavePng ()
{
  string prefsName = $"renderCount_{Application.productName}";
  int take = 0;
#if UNITY_EDITOR
  take = UnityEditor.EditorPrefs.GetInt(prefsName, 0);
#endif
  string pngName = $"{folderName}/{videoFrame:0000} shot{take}.png";
#if UNITY_EDITOR
  UnityEditor.EditorPrefs.SetInt(prefsName, ++take);
#endif
  var pngShot = textureTransparentBackground.EncodeToPNG();
  File.WriteAllBytes(pngName, pngShot);
}
